    ...
    <el-card v-if="review">
      ...
      <div class="review-images">
        ...
        <div class="images-box">
          <img v-for="(url, index) in review.images" 
          :key="index" 
          :src="url" 
          alt="图片"
          ></img>
...
const handleApprove = () => {
  ElMessageBox.confirm('确定要批准该审核吗?', '提示', {
    ...
    try {
      const updatedReview = await reviewStore.approveReviewItem(reviewId);
...
const confirmReject = async () => {
  ...
  await rejectFormRef.value.validate(async (valid) => {
    if (valid) {
      actionLoading.value = true;
      try {
        const updatedReview = await reviewStore.rejectReviewItem(reviewId, rejectForm.value.reason);
        review.value = updatedReview;
